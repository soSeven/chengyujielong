...
class AllTaskTableCell: TableViewCell {
    ...
    lazy var textLbl: UILabel = {
        let lbl = UILabel()
        lbl.textColor = .init(hex: "#A6571F")
        lbl.font = .init(style: .regular, size: 10.uiX)
        lbl.text = "累计闯1关，即可获得奖励"
        return lbl
    }()
    
    private let item1 = AllTaskTableCellItem(imgName: "task_img_package")
    private let item2 = AllTaskTableCellItem(imgName: "task_img_coin")
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        let sH = UIStackView(arrangedSubviews: [item1, item2])
        sH.axis = .horizontal
        sH.alignment = .center
        sH.distribution = .equalSpacing
        sH.spacing = 8.uiX
        
        let s = UIStackView(arrangedSubviews: [titleLbl, textLbl, sH])
        s.axis = .vertical
        s.alignment = .leading
    ...
    func bind(to viewModel: AllTaskCellViewModel) {
        titleLbl.text = "获得“\(viewModel.text ?? "")”头衔"
        setupProgress(num: viewModel.nowCount, total: viewModel.min)
        textLbl.text = "累计闯\(viewModel.min ?? 0)关，即可获得奖励"
        if viewModel.money > 0 {
            item1.isHidden = false
            item1.textLbl.text = "\(viewModel.money ?? 0)元"
        } else {
            item1.isHidden = true
        }
        if viewModel.goldCoin > 0 {
            item2.isHidden = false
            item2.textLbl.text = "\(viewModel.goldCoin ?? 0)"
        } else {
            item2.isHidden = true
        }
...
private class AllTaskTableCellItem: UIView {
